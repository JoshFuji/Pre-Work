...
function addGamesToPage(games) {
    ...
    for (let i = 0; i < games.length; i ++){
        ...
        gameCard.innerHTML = `
            <img src="${game.img}" alt="Cover image for ${game.name}" class="game-img">
            <h3>${game.name}</h3>
            <p>${game.description}</p>
            <p>
                <strong>${game.backers.toLocaleString()}</strong>
            </p>
            <p>$${game.pledged.toLocaleString()}</p>
        `;
...
const unfundedCount = GAMES_JSON.reduce((acc, game) => acc + (game.pledged < game.goal ? 1 : 0), 0);

// create a string that explains the number of unfunded games using the ternary operator
const displayStr = 
     `A total of $${GAMES_JSON.reduce((s, g) => s + g.pledged, 0).toLocaleString()}
     has been raised for ${GAMES_JSON.length} games.
     Currently, ${unfundedCount} ${unfundedCount === 1 ? "game remains" : "games remain"}
